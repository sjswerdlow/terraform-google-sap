...
variable "db2_sid_size" {
  type = number
  description = "Size in GB of the /db2/[DBSID] directory, the root directory of the database instance."
  default = 8
  validation {
...
variable "db2_home_size" {
  type = number
  description = "Size in GB of the /db2/db2[DBSID] directory, the home directory of the database instance."
  default = 8
  validation {
    condition = var.db2_home_size >= 8
    error_message = "Home drive must be at least 8GB."
  }
}

variable "db2_dump_size" {
  type = number
  description = "Size in GB of the /db2/[DBSID]/db2dump directory, this directory holds dump files from DB2 which may be useful for diagnosing problems."
  default = 8
  validation {
    condition = var.db2_dump_size >= 8
    error_message = "Dump drive must be at least 8GB."
  }
}

variable "db2_sap_tmp_size" {
  type = number
  description = "Size in GB of the /db2/[DBSID]/saptmp directory, which holds the database temporary table space."
  default = 8
  validation {
...
variable "db2_sap_data_size" {
  type = number
  description = "Size in GB of the /db2/[DBSID]/sapdata directory, which holds the database data files."
  default = 30
  validation {
...
variable "db2_log_size" {
  type = number
  description = "Size in GB of the /db2/[DBSID]/logdir directory, which holds the database transaction logs."
  default = 8
  validation {
...
variable "db2_backup_size" {
  type = number
  description = "Size in GB of the /db2backup directory, if set to 0, the volume will not be created."
  default = 0
  validation {
...
variable "primary_startup_url" {
  type = string
  default = "curl -s BUILD.TERRA_SH_URL/sap_db2/startup.sh | bash -s BUILD.TERRA_SH_URL"
