...
GCS_CONTINUOUS_BUCKET="cloudsapdeploytesting/deploymentmanager/continuous"
...
TERRAFORM_URL_CONTINUOUS="https://www.googleapis.com/storage/v1/${TERRAFORM_GCS_CONTINUOUS_BUCKET}"
...
RELEASED_TERRAFORM_MODULES=("sap_hana" "sap_hana_ha" "sap_hana_scaleout", "sap_nw", "sap_nw_ha")
...
if [[ "${GCS_FOLDER}" =~ ^("build_continuous_testing"|"nightly")$ ]]; then
  ...
  TERRAFORM_GCS_CONTINUOUS_BUCKET="cloudsapdeploytesting/terraform/continuous"
  TERRAFORM_PREFIX=""
  TERRAFORM_URL="https://storage.googleapis.com/${TERRAFORM_GCS_BUCKET}"
  TERRAFORM_URL_LATEST="https://storage.googleapis.com/${TERRAFORM_GCS_CONTINUOUS_BUCKET}"
  GCE_STORAGE_REPO_SUFFIX=""
  PACEMAKER_ALIAS_COPY="curl ${RESOURCE_URL_CONTINUOUS}/pacemaker-gcp/alias -o"
  PACEMAKER_ROUTE_COPY="curl ${RESOURCE_URL_CONTINUOUS}/pacemaker-gcp/route -o"
  PACEMAKER_STONITH_COPY="curl ${RESOURCE_URL_CONTINUOUS}/pacemaker-gcp/gcpstonith -o"
  GSUTIL_PUBLIC_OPT="-a public-read"
fi
...
build() {
  ...
  grep -rl SAP_LIB_HDBFR . | xargs ${SED_CMD} -e '/SAP_LIB_HDBFR/{r lib/sap_lib_hdbfr.sh' -e 'd' -e '}'
...
make_github_files() {
  ...
  for release in ${RELEASED_TERRAFORM_MODULES[@]}; do
    find ".build_dmtemplates/${release}/terraform/${release}/" -name "*.tf" | xargs cp -t "${dir_prefix}modules/${release}/"
  done
}
...
echo "GCS_CONTINUOUS_BUCKET=${GCS_LATEST_BUCKET}"
echo "GCS_LATEST_BUCKET=${GCS_LATEST_BUCKET}"
echo "RESOURCE_URL=${RESOURCE_URL}"
echo "RESOURCE_URL_CONTINUOUS=${RESOURCE_URL_LATEST}"
...
echo "TERRAFORM_GCS_CONTINUOUS_BUCKET=${TERRAFORM_GCS_CONTINUOUS_BUCKET}"
